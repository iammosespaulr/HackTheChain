...
function SignUpPage() {
	...
	async function handleSubmit() {
		...
		axios
		.post(`https://35.208.156.236:4000/api/users/new`, { signup })
		.then((response) => {
	...
	return (
		<Row className="hide-overflow">
			<Col md="5">
				...
				<Row className="padding-left-column">
					<p
						className="hedera-label"
						data-tip="Ensures better security and transparency through Hedera Consensus Service"
					>
						<b>1.</b>Create Hedera Account
					</p>
					<ReactTooltip />
					<a href="https://portal.hedera.com/register" target="_blank">
						<Button variant="contained" size="large" fontSize="100px" className="hedera-button">
							Go To Hedera
						</Button>
					</a>
				</Row>
				<Row className="padding-left-column">
					...
					<TextField
						required
						id="standard-basic"
						className="second-column-signup"
						label="PRIVATE KEY"
						onChange={handleChangePrivateKey}
					/>
				</Row>
				<Row className="padding-left-column">
					<p className="hedera-label part-two">
						<b>2.</b>Fill in information
					</p>
				</Row>
				<Row className="padding-left-column">
					<TextField
				...
				<Row>
					<Col md="12">
						<Button variant="contained" className="login-submit-button" onClick={handleSubmit}>
							Sign Up
						</Button>
					</Col>
				</Row>
